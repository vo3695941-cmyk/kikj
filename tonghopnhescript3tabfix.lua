...
local function setupPage(page)
    page.Position = UDim2.new(0, 0, 0.35, 0)
...
-----------------------------------------------------------
-- PHẦN 1: TAB BAY (FIXED LỖI ĐI LỆCH & TÔNG TƯỜNG)
-----------------------------------------------------------
local speeds = 1
local nowe = false
        ...
        task.spawn(function()
            while nowe do
                ...
                if moveDir.Magnitude > 0 then
                    -- CÔNG THỨC MỚI: Tính vận tốc dựa trên Joystick + Camera hướng nhìn
                    -- Khi quẹt trái/phải/lùi, nó sẽ đi thẳng tắp theo góc camera hiện tại
                    local look = camera.CFrame.LookVector
                    local right = camera.CFrame.RightVector
                    
                    -- Kết hợp hướng của Joystick vào không gian 3D
                    local targetVelocity = (look * -moveDir.Z + right * moveDir.X).Unit * (speeds * 50)
                    
                    -- Nếu bạn nhìn lên trời, LookVector sẽ kéo bạn lên cao tự động
...
-----------------------------------------------------------
-- PHẦN 2 & 3: CÔNG CỤ & SCRIPT (Giữ nguyên bản ổn định của bạn)
-----------------------------------------------------------
local createdParts = {}
local ghosting = false
local function createTool(txt, func)
    local b = Instance.new("TextButton", ToolPage)
    b.Size = UDim2.new(0.9, 0, 0, 30); b.Text = txt; b.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
    Instance.new("UICorner", b); b.MouseButton1Click:Connect(func)
end
createTool("MỞ F3X", function() loadstring(game:GetObjects("rbxassetid://6695644299").Source)() end)
createTool("TẠO KHỐI", function() 
    ...
    p.Anchored = true; p.BrickColor = BrickColor.new("Bright orange"); table.insert(createdParts, p)
end)
createTool("XUYÊN/CỨNG KHỐI", function() 
    ghosting = not ghosting
    for _, p in pairs(createdParts) do if p and p.Parent then p.CanCollide = not ghosting; p.Transparency = ghosting and 0.5 or 0 end end
end)
createTool("XÓA TẤT CẢ KHỐI", function() for _, p in pairs(createdParts) do if p then p:Destroy() end end createdParts = {} end)

local infJumpActive, holdingJump = false, false
...
addToggle("INF JUMP (ĐÈ)", ScriptPage, function(s) infJumpActive = s end)
...
addToggle("ESP PLAYER", ScriptPage, function(s)
    if s then for _, p in pairs(game.Players:GetPlayers()) do if p ~= player and p.Character and p.Character:FindFirstChild("Head") then
        ...
        task.spawn(function() while b.Parent do l.Text = p.Name .. "\n[" .. math.floor((player.Character.HumanoidRootPart.Position - p.Character.HumanoidRootPart.Position).Magnitude) .. "m]"; task.wait(0.5) end end)
    end end else for _, p in pairs(game.Players:GetPlayers()) do if p.Character and p.Character.Head:FindFirstChild("XESP") then p.Character.Head.XESP:Destroy() end end end
end)
