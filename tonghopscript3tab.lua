...
main.Name = "XNEO_HUB_FINAL_FIXED"
...
ToolTabBtn = createTabBtn("Công Cụ", UDim2.new(0.33, 0, 0, 0))
...
local function setupPage(page)
    ...
    if page:IsA("ScrollingFrame") then
        page.CanvasSize = UDim2.new(0, 0, 2.5, 0) -- Tăng độ dài để chứa hết nút
...
-----------------------------------------------------------
-- PHẦN 3: TAB SCRIPT (SPEED, INF JUMP, ESP, FLOAT, NOCLIP)
-----------------------------------------------------------
...
addScriptBtn("SPEED (50)", function() speaker.Character.Humanoid.WalkSpeed = 50 end)
addScriptBtn("NORMAL SPEED", function() speaker.Character.Humanoid.WalkSpeed = 16 end)

local iJump = false
game:GetService("UserInputService").JumpRequest:Connect(function() if iJump then speaker.Character.Humanoid:ChangeState("Jumping") end end)
addScriptBtn("INF JUMP", function() iJump = not iJump end)

addScriptBtn("ESP PLAYER", function()
    for _, p in pairs(game.Players:GetPlayers()) do
        if p ~= speaker and p.Character and p.Character:FindFirstChild("Head") then
            ...
            b.Size = UDim2.new(0, 100, 0, 50); b.AlwaysOnTop = true; b.Name = "XESP"
            local l = Instance.new("TextLabel", b)
            l.Size = UDim2.new(1, 0, 1, 0); l.BackgroundTransparency = 1
            l.TextColor3 = Color3.fromRGB(255, 0, 0); l.TextSize = 10; l.TextStrokeTransparency = 0
            task.spawn(function()
                while p.Character and p.Character:FindFirstChild("HumanoidRootPart") do
...
local fOn = false
addScriptBtn("FLOAT", function()
    fOn = not fOn
    local r = speaker.Character.HumanoidRootPart
    if fOn then
        local bv = Instance.new("BodyVelocity", r); bv.Name = "XFloat"; bv.MaxForce = Vector3.new(0, 9e9, 0); bv.Velocity = Vector3.new(0, 0, 0)
    else
        if r:FindFirstChild("XFloat") then r.XFloat:Destroy() end
    end
end)

local noclip = false
addScriptBtn("NOCLIP (XUYÊN TƯỜNG)", function()
    noclip = not noclip
...
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "XNEO HUB V4", Text = "Chào mừng " .. speaker.Name .. "!", Duration = 5})
